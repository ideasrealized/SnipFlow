...
function isCursorInTriggerZone(
  cursorPos: Electron.Point,
  position: string,
  triggerSize: number,
  ...
  switch (position) {
    case 'top-left':
      return (
        cursorScreenX >= areaX &&
        cursorScreenX < areaX + triggerSize &&
        cursorScreenY >= areaY &&
        cursorScreenY < areaY + triggerSize
      );
    case 'top-right':
      return (
        cursorScreenX > areaX + areaWidth - triggerSize &&
        cursorScreenX <= areaX + areaWidth &&
        cursorScreenY >= areaY &&
        cursorScreenY < areaY + triggerSize
      );
    case 'bottom-left':
      return (
        cursorScreenX >= areaX &&
        cursorScreenX < areaX + triggerSize &&
        cursorScreenY > areaY + areaHeight - triggerSize &&
        cursorScreenY <= areaY + areaHeight
      );
    case 'bottom-right':
      return (
        cursorScreenX > areaX + areaWidth - triggerSize &&
    ...
    case 'left-center':
      return (
        cursorScreenX >= areaX &&
        cursorScreenX < areaX + triggerSize &&
        cursorScreenY > areaY + areaHeight * 0.3 &&
        cursorScreenY < areaY + areaHeight * 0.7
      );
    case 'right-center':
      return (
        cursorScreenX > areaX + areaWidth - triggerSize &&
...
function isMouseOverOverlayWindow(
  cursorPos: Electron.Point,
  overlayBounds: Electron.Rectangle | null
): boolean {
  if (!overlayWindow || !overlayWindow.isVisible() || !overlayBounds)
    return false;
  return (
    cursorPos.x >= overlayBounds.x &&
    cursorPos.x <= overlayBounds.x + overlayBounds.width &&
    cursorPos.y >= overlayBounds.y &&
    cursorPos.y <= overlayBounds.y + overlayBounds.height
  );
}
...
function startMouseTracking(currentSettings: Settings) {
  ...
  const activationDelay =
    currentSettings.edgeHover.delay || DEFAULT_EDGE_HOVER_ACTIVATION_DELAY;
  logger.info(
    `[main.ts] Starting mouse tracking. Position: ${currentSettings.edgeHover.position}, Delay: ${activationDelay}ms, Size: ${currentSettings.edgeHover.triggerSize}px`
  );
  ...
  mouseMoveInterval = setInterval(() => {
    ...
    const isInCurrentActivationZone = isCursorInTriggerZone(
      cursorPos,
      position,
      triggerSize,
      workArea
    ...
    if (isInCurrentActivationZone && !isHovering && !mouseOverOverlay) {
      if (overlayState === 'hidden' || overlayState === 'hiding') {
        logger.info(
          '[main.ts] startMouseTracking: Initiating show sequence. Setting overlayState to showing.'
        );
        ...
        logger.info(
          `[main.ts] Mouse entered ${position} zone. Setting hoverTimeout for ${activationDelay}ms. state: ${overlayState}`
        );
        if (hoverTimeout) clearTimeout(hoverTimeout);

        hoverTimeout = setTimeout(() => {
          const latestCursorPos = screen.getCursorScreenPoint();
          const stillInZone = isCursorInTriggerZone(
            latestCursorPos,
            position,
            triggerSize,
            workArea
          );
          const latestOverlayBounds =
            overlayWindow && overlayWindow.getBounds();
          const stillNotOverOverlay = !isMouseOverOverlayWindow(
            latestCursorPos,
            latestOverlayBounds
          );
          ...
          if (
            ...
            logger.info(
              '[main.ts] hoverTimeout: Conditions met. Positioning and showing overlay.'
            );
            ...
            overlayWindow?.webContents.send('overlay:show', {
              position: lastUsedEdgeHoverPosition,
              screen: screen.getDisplayMatching(
                latestOverlayBounds || workArea
              ),
            });

            logger.info(
              '[main.ts] hoverTimeout: Overlay shown. Setting overlayState to visible.'
            );
            overlayState = 'visible';
          } else {
            logger.info(
              `[main.ts] hoverTimeout: Show conditions NOT met (isHovering: ${isHovering}, stillInZone: ${stillInZone}, stillNotOverOverlay: ${stillNotOverOverlay}, overlayState: ${overlayState}).`
            );
            if (overlayState === 'showing') {
              logger.info(
                '[main.ts] hoverTimeout: Resetting overlayState to hidden because show attempt failed.'
              );
              overlayState = 'hidden';
            }
            isHovering = false;
    ...
    // HIDE INITIATION LOGIC
      ...
      (overlayIsVisible &&
        !mouseOverOverlay &&
        lastUsedEdgeHoverPosition &&
        !isCursorInTriggerZone(
          cursorPos,
          lastUsedEdgeHoverPosition,
          triggerSize,
          workArea
      ...
      if (isHovering) {
        logger.info(
          `[main.ts] Mouse left ${lastUsedEdgeHoverPosition || 'an active'} zone or conditions changed. Clearing hoverTimeout, resetting isHovering.`
        );
        isHovering = false;
        if (hoverTimeout) {
        ...
        if (overlayState === 'showing') {
          logger.info(
            "[main.ts] Mouse left zone while trying to show (state was 'showing'). Resetting overlayState to hidden."
          );
          overlayState = 'hidden';
        }
      }
      ...
      if (overlayIsVisible && !mouseOverOverlay) {
        ...
        if (
          ...
          logger.info(
            `[main.ts] Conditions met to initiate overlay hide (mouse left ${relevantZoneToCheck}). Scheduling hide with delay.`
          );
          setTimeout(() => {
            ...
            const stillNotInRelevantZone = !isCursorInTriggerZone(
              latestCursorPosAfterHideDelay,
              relevantZoneToCheck,
              triggerSize,
              workAreaAfterHideDelay
            );
            const latestOverlayIsVisibleAfterDelay =
              overlayWindow && overlayWindow.isVisible();
            const latestOverlayBoundsAfterDelay =
              latestOverlayIsVisibleAfterDelay
                ? overlayWindow && overlayWindow.getBounds()
                : null;
            const stillNotOverOverlayAfterHideDelay = !isMouseOverOverlayWindow(
              latestCursorPosAfterHideDelay,
              latestOverlayBoundsAfterDelay
            );

            if (
              overlayWindow &&
              latestOverlayIsVisibleAfterDelay &&
              stillNotOverOverlayAfterHideDelay &&
              stillNotInRelevantZone
            ) {
              logger.info(
                '[main.ts] Mouse confirmed away after hide-delay. Calling hideOverlay("mouse_leave").'
              );
              hideOverlay('mouse_leave');
            } else {
              logger.info(
                '[main.ts] Hide conditions no longer met after mouse-leave delay. Aborting hide request.'
              );
            }
...
function positionOverlayAtPosition(activatedPosition: string) {
  if (!overlayWindow) {
    logger.warn(
      '[main.ts] positionOverlayAtPosition called but overlayWindow is null.'
    );
    return;
  }
  ...
  const overlayWidth =
    Array.isArray(overlaySizeArray) && typeof overlaySizeArray[0] === 'number'
      ? overlaySizeArray[0]
      : 400; // Default width
  const overlayHeight =
    Array.isArray(overlaySizeArray) && typeof overlaySizeArray[1] === 'number'
      ? overlaySizeArray[1]
      : 500; // Default height
  ...
  const preferredY =
    currentSettings.overlay.y !== undefined
      ? currentSettings.overlay.y
      : workArea.y + workArea.height * 0.1;
  ...
  y = Math.max(
    workArea.y,
    Math.min(y, workArea.y + workArea.height - overlayHeight)
  );
  ...
  switch (activatedPosition) {
    ...
    default:
      logger.warn(
        `[main.ts] Unknown position '${activatedPosition}'. Defaulting to preferred side: ${preferredSide}.`
      );
      x =
        preferredSide === 'left'
          ? workArea.x
  ...
  logger.info(
    `[main.ts] Positioning overlay at x: ${Math.round(x)}, y: ${Math.round(y)} for position: ${activatedPosition}`
  );
...
function createWindow() {
  ...
  mainWindow.webContents.on('render-process-gone', (e, d) =>
    logger.error('Main render gone:', d)
  );
...
function createOverlayWindow() {
  ...
  overlayWindow = new BrowserWindow({
    ...
    y: workArea.y + 100, // Off-screen initially
    frame: false,
    transparent: true,
    resizable: false,
    alwaysOnTop: true,
  ...
  overlayWindow.on('blur', () => {
    logger.info(
      `[main.ts] Overlay window blurred. Current state: ${overlayState}`
    );
    if (overlayState === 'visible') {
      logger.info(
        '[main.ts] Blur event: Overlay was visible, calling hideOverlay("blur_event").'
      );
      hideOverlay('blur_event');
    } else {
      logger.info(
        `[main.ts] Blur event: Overlay not hiding via blur as state is ${overlayState}.`
      );
    }
  });
...
function setupIpcHandlers() {
  ...
  ipcMain.handle('get-settings', async () => {
    try {
      ...
      logger.info(
        `[main.ts] IPC: get-settings returning: ${JSON.stringify(settings).substring(0, 100)}...`
      );
      return settings;
    } catch (error) {
  ...
  ipcMain.handle(
    ...
    async (event, newSettings: Partial<Settings>) => {
      try {
        logger.info(
          `[main.ts] IPC: save-settings received with ${JSON.stringify(newSettings).substring(0, 100)}...`
        );
  ...
  ipcMain.handle('get-chain-by-id', async (event, id: number) => {
    ...
    try {
      ...
      if (chain) {
        logger.info(
          `[main.ts] IPC: get-chain-by-id found chain: ${chain.name}`
        );
        return chain;
      } else {
        logger.warn(
          `[main.ts] IPC: get-chain-by-id chain not found for ID: ${id}`
        );
        return null;
      }
    } catch (error) {
      logger.error(
        `[main.ts] IPC: Error in get-chain-by-id for ID ${id}:`,
        error
      );
      return null;
    }
  ...
  ipcMain.handle('get-chain-by-name', async (event, name: string) => {
    ...
    try {
      ...
      if (chain) {
        logger.info(
          `[main.ts] IPC: get-chain-by-name found chain: ${chain.name}`
        );
        return chain;
      } else {
        logger.warn(
          `[main.ts] IPC: get-chain-by-name chain not found for name: ${name}`
        );
        return null;
      }
    } catch (error) {
      logger.error(
        `[main.ts] IPC: Error in get-chain-by-name for name ${name}:`,
        error
      );
      return null;
    }
  ...
  ipcMain.handle(
    'update-chain',
    async (_, id: number, data: Partial<Omit<Chain, 'id'>>) => {
      try {
  ...
  ipcMain.handle('get-clipboard-history', async () => {
    ...
    try {
      ...
      logger.info(
        `[main.ts] IPC: get-clipboard-history returning ${history.length} items.`
      );
      return history;
    } catch (error) {
  ...
  ipcMain.handle(
    'pin-clipboard-item',
    async (event, id: string, pinned: boolean) => {
      logger.info(
        `[main.ts] IPC: pin-clipboard-item received for ID: ${id}, Pinned: ${pinned}`
      );
      try {
        const result = await dbPinClipboardItem(id, pinned);
        logger.info(
          `[main.ts] IPC: pin-clipboard-item successful for ID: ${id}`
        );
        return result;
      } catch (error) {
        logger.error(
          `[main.ts] IPC: Error in pin-clipboard-item for ID ${id}:`,
          error
        );
        return { success: false, error: (error as Error).message };
      }
  ...
  ipcMain.handle('process-text-with-chain', async (event, text: string) => {
    try {
      logger.info(
        `[main.ts] IPC: process-text-with-chain received with text length: ${text.length}`
      );
      const result = await processTextWithChain(
        ...
        async (
          ...
          const mainChoices: Choice[] = options.map(opt => ({
            id: opt.label,
            title: opt.label,
            body: opt.text,
          }));
      ...
      logger.info(
        `[main.ts] IPC: process-text-with-chain returning result length: ${result.length}`
      );
      return result;
    } catch (error) {
  ...
  ipcMain.on('overlay:ready', () => {
    ...
    if (overlayState === 'showing') {
      logger.info(
        '[main.ts] overlay:ready: Overlay was showing, ensuring state is visible.'
      );
      overlayState = 'visible';
    }
  });
  ...
  ipcMain.on('request-main-window-focus', () => {
    logger.info(
      '[main.ts] IPC: request-main-window-focus received from renderer.'
    );
  ...
  ipcMain.on('hide-overlay', () => {
    logger.info(
      `[main.ts] IPC hide-overlay received. Current state: ${overlayState}`
    );
  ...
  ipcMain.on('overlay:hidden-ack', () => {
    logger.info(
      `[main.ts] ✅ RECEIVED overlay:hidden-ack. Current state: ${overlayState}`
    );
    if (
      overlayWindow &&
  ...
  const mainChoiceProvider: MainChoiceProvider = {
    presentChoice: async (choices: Choice[]) => {
      return new Promise(resolve => {
        ...
        if (!target) {
          logger.error('No window for choice');
          resolve(null);
          return;
        }
        target.webContents.send('present-choice', choices);
        ipcMain.once('choice-response', (_, choiceId: string | null) =>
          resolve(choiceId)
        );
      });
    },
  ...
  const mainInputProvider: MainInputProvider = {
    presentInput: async (promptText: string, initialValue?: string) => {
      return new Promise(resolve => {
        const target = overlayWindow?.isVisible() ? overlayWindow : mainWindow;
        if (!target) {
          logger.error('No window for input');
          resolve(null);
          return;
        }
        target.webContents.send('present-input', {
          prompt: promptText,
          initialValue,
        });
        ipcMain.once('input-response', (_, value: string | null) =>
          resolve(value)
        );
      });
    },
  ...
  ipcMain.handle(
    'execute-chain',
    async (_, chainId: number, dynamicInput?: string) => {
      try {
        ...
        logger.info(
          `[main.ts] Executing chain '${chain.name}' (ID: ${chainId}). Options count: ${chain.options.length}`
        );
        ...
        if (chain.options.length === 0) {
          logger.warn(
            `Chain "${chain.name}" has no options. Nothing to execute.`
          );
          ...
          logger.info(
            `Chain "${chain.name}" has multiple options. Presenting choices.`
          );
          const chosenOptionId = await mainChoiceProvider.presentChoice(
            choicesForMainProvider
          );
          ...
          if (chosenOptionId) {
            ...
            logger.info(
              `User selected option ID: ${chosenOptionId}. Using its body.`
            );
          } else {
            logger.info(
              `User cancelled choice for chain "${chain.name}". Execution halted.`
            );
        ...
        if (dynamicInput && contentToProcess.includes('[?]')) {
          ...
          logger.info(
            `Replaced [?] with dynamic input. New content (first 100): ${contentToProcess.substring(0, 100)}`
          );
        } else if (dynamicInput) {
          logger.info(
            `Dynamic input provided but no [?] placeholder found in content for chain ${chain.name}.`
          );
        }

        // Adapter for processTextWithChain's choiceProvider
        const adaptedChoiceProviderForTextProcessing = async (
          question: string,
          options: { label: string; text: string }[]
        ) => {
        ...
        const adaptedInputProviderForTextProcessing = async (
          promptText: string
        ) => {
          const result = await mainInputProvider.presentInput(promptText);
        ...
        logger.info(
          `Processing content for chain "${chain.name}" with processTextWithChain. Content (first 100): ${contentToProcess.substring(0, 100)}`
        );
        ...
        logger.info(
          `Chain "${chain.name}" final output (first 100): ${finalOutput.substring(0, 100)}`
        );
  ...
  ipcMain.handle('create-snippet', async (_, content: string) =>
    dbCreateSnippet(content)
  );
  ipcMain.handle('update-snippet', async (_, id: number, content: string) =>
    dbUpdateSnippet(id, content)
  );
...
function hideOverlay(source: string) {
  if (overlayState === 'visible' || overlayState === 'showing') {
    logger.info(
      `[main.ts] hideOverlay (source: ${source}): Requesting hide. Current state: ${overlayState}. Setting to hiding.`
    );
    overlayState = 'hiding';
    isHovering = false;
    if (hoverTimeout) {
    ...
    if (overlayWindow && !overlayWindow.isDestroyed()) {
      ...
      setTimeout(() => {
        if (overlayState === 'hiding') {
          logger.warn(
            `[main.ts] Fallback Safety: No overlay:hidden-ack received within 2000ms for source '${source}'. Force hiding.`
          );
          if (
            overlayWindow &&
            !overlayWindow.isDestroyed() &&
          ...
          lastUsedEdgeHoverPosition = null;
          logger.info(
            '[main.ts] Fallback Safety: Overlay state forced to hidden.'
          );
        }
      }, 2000);
    } else {
      logger.warn(
        `[main.ts] hideOverlay (source: ${source}): overlayWindow not available to send "overlay:hide"`
      );
      if (overlayState === 'hiding') {
        logger.warn(
          `[main.ts] hideOverlay (source: ${source}): OverlayWindow gone, forcing state to hidden.`
        );
        overlayState = 'hidden';
        isHovering = false;
        lastUsedEdgeHoverPosition = null;
    ...
    logger.info(
      `[main.ts] hideOverlay (source: ${source}): Called but overlay not visible or showing. State: ${overlayState}. No action.`
    );
  }
}
